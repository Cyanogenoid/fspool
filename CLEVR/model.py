...
class RelationalLayer(RelationalLayerBase):
    def __init__(self, in_size, out_size, qst_size, hyp, extraction=False):
        ...
        self.version = hyp['version']
        self.fspool_pieces = hyp.get('fspool_pieces', 20)
    ...
    def forward(self, x, qst):
        ...
        if self.version == 'rn':
            ...
            x_ = x_full.view(b * d, self.in_size)
        ...
        for idx, (g_layer, g_layer_size) in enumerate(zip(self.g_layers, self.g_layers_size)):
            if idx==self.quest_inject_position:
                ...
                if self.version == 'rn':
                    ...
                    x_img = x_.view(b,d,in_size)
                    x_concat = torch.cat([x_img,qst],2)

                # h layer
                if self.version == 'rn':
                    x_ = x_concat.view(b*d*d,in_size+self.qst_size)
                else:
                    x_ = x_concat.view(b*d,in_size+self.qst_size)
                x_ = g_layer(x_)
                x_ = F.relu(x_)
            else:
                x_ = g_layer(x_)
        ...
        if self.version == 'rn':
            ...
            n_objects = (x_full.transpose(1, 2).abs().sum(1) != 0).sum(1).long()
            # need to multiply by 12 to have same scale as original model
            x_g = self.pool(x_g, n_objects.data)[0] * 12
